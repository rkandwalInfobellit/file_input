...
const releaseSlice = createSlice({
  ...
  reducers: {
    ...
    resetReleaseList(state) {
      state.releases = [];
      state.releasePage = 1;
      state.releaseHasMore = false;
      state.fetchStatus = "idle";
    },
    resetDraftFilesStatus(state) {
      state.draftFetchStatus = "idle";
    },
    setDraftPage(state, action) {
      state.draftPage = action.payload;
    },
  },
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchReleases.fulfilled, (state, action) => {
        ...
        state.releases =
          page === 1 ? incoming : [...state.releases, ...incoming];
      })
      ...
      // fetchDraftFiles
      .addCase(fetchDraftFiles.pending, (state) => {
        state.draftFetchStatus = "loading";
        state.error = null;
      })
      ...
      .addCase(fetchDraftFiles.rejected, (state, action) => {
        state.draftFetchStatus = "failed";
        state.error = action.payload;
      })
