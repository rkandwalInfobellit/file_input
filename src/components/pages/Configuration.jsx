...
import { fetchCategories, deleteCategory, toggleCategoryActive } from "@/store/slice/category.slice"
import {
  selectCategories,
...
export default function Configuration() {
  ...
  function load(pg = pageIndex + 1, q = search) {
    dispatch(fetchCategories({ page: pg, limit: pageSize, ...(q ? { search: q } : {}) }))
  }

  // Reload when page or pageSize changes
  useEffect(() => { load(pageIndex + 1) }, [pageIndex, pageSize])  // eslint-disable-line
  ...
  function handleEdit(category)  { setEditRule(category); setSheetOpen(true) }
    ...
    <section className="flex flex-col gap-4 px-4 py-1">
      <div>
        ...
        <p className="text-sm text-muted-foreground max-w-2xl">
          Define who can approve files based on cloud, application, and file category.
        </p>
      </div>
      ...
      <div className="rounded-lg border bg-card">
        <div className="flex items-center justify-between px-5 py-4 border-b">
          <div className="flex items-center gap-2">
            ...
            <div>
              <div className="font-semibold text-sm">File Category Rules</div>
