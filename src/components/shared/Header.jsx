...
/**
 * The single global header used across the app. Renders the brand lockup
 * on the left and a compact utility cluster (search, notifications,
 * help, account) on the right. Meant to be rendered once, above the
 * sidebar/main content split.
 */
export default function Header({ userName = "testuser@infobellit.com", notificationCount = 0 }) {
  ...
  return (
    <header className="flex h-[62px] items-center justify-between border-b border-border bg-foreground px-4">
      {/* Brand */}
      ...
      <div className="flex items-center gap-1.5">
        ...
        <div className="relative">
          ...
          >
            ...
            <span className="hidden text-sm text-primary-foreground md:inline">{userName}</span>
            <ChevronDown
              className={cn(
          ...
          {accountOpen && (
            <>
              ...
              <div className="absolute right-0 z-20 mt-2 w-48 rounded-lg border border-border bg-popover py-1 shadow-lg">
                ...
                <button className="block w-full px-3 py-2 text-left text-sm text-destructive hover:bg-accent">
                  Sign out
                </button>
              </div>
            </>
